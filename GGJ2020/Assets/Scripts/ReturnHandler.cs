...
// Handles the returning of player when time ran out
...
public class ReturnHandler : MonoBehaviour
{
    ...
    public void ReturnFromDungeon()
    {
        ...
        seq.AppendCallback(() => { DialogueSystem.instance.StartDialogue(4, false); DialogueSystem.instance.OnDialogueEnd.AddListener(ShowButtons); });
        seq.PlayForward();
    }

    public void TalkToItem()
    {
        DialogueSystem.instance.StartDialogue(4,false);
        HideButtons();
    ...
    public void ReturnToDungeon()
    {
        ...
        Camera mainCamera = Camera.main;
        Sequence seq = DOTween.Sequence();
        seq.AppendCallback(() => DOTween.To(() => mainCamera.transform.position, x => mainCamera.transform.position = x, new Vector3(0, -4, mainCamera.transform.position.z), 3.0f));
        seq.AppendInterval(2.0f);
        seq.Append(DOTween.To(() => mainCamera.orthographicSize, x => mainCamera.orthographicSize = x, 3.0f, 3.0f));
        seq.AppendCallback(() => SceneSystem.instance.FadeToNextScene("RandomGenerator"));
        seq.PlayForward();
    }
